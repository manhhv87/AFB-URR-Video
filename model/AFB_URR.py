...
class Refine(nn.Module):
    ...
    def forward(self, f, pm):
        ...
        m = s + NF.interpolate(pm, scale_factor=self.scale_factor,
...
class Matcher(nn.Module):
    ...
    def forward(self, feature_bank, q_in, q_out):
        ...
        for i in range(0, feature_bank.obj_n):
            ...
            try:
                p = torch.matmul(feature_bank.keys[i].transpose(
                    0, 1), q_in) / math.sqrt(d_key)  # THW, HW
                ...
                p = torch.matmul(key_cpu.transpose(
                    0, 1), q_in_cpu) / math.sqrt(d_key)  # THW, HW
                p = NF.softmax(p, dim=1)  # bs, bank_n, HW
                mem = torch.matmul(value_cpu, p).to(device)  # bs, D_o, HW
                p = p.to(device)
                print('\tLine 158. GPU out of memory, use CPU',
                      f'p size: {p.shape}')
            ...
            if self.update_bank:
                try:
                    ...
                    bank_cnt = torch.where(
                        p > self.thres_valid, ones, zeros).sum(dim=2)[0]
                except RuntimeError as e:
                    device = p.device
                    p = p.cpu()
                    ones = torch.ones_like(p)
                    zeros = torch.zeros_like(p)
                    bank_cnt = torch.where(p > self.thres_valid, ones, zeros).sum(dim=2)[
                        0].to(device)
                    print('\tLine 170. GPU out of memory, use CPU',
                          f'p size: {p.shape}')

                feature_bank.info[i][:, 1] += torch.log(bank_cnt + 1)

        mem_out_tensor = torch.stack(mem_out_list, dim=0).transpose(
            0, 1)  # bs, obj_n, dim, pixel_n
...
class Decoder(nn.Module):
    def __init__(self, device):  # mdim_global = 256
        ...
        self.convFM = nn.Conv2d(
            1024, mdim_global, kernel_size=3, padding=1, stride=1)
        ...
        self.pred2 = nn.Conv2d(
            mdim_global, 2, kernel_size=3, padding=1, stride=1)

        # Local
        self.local_avg = nn.AvgPool2d(
            local_size, stride=1, padding=local_size // 2)
        self.local_max = nn.MaxPool2d(
            local_size, stride=1, padding=local_size // 2)
        ...
        self.local_pred2 = nn.Conv2d(
            mdim_local, 2, kernel_size=3, padding=1, stride=1)
    ...
    def forward(self, patch_match, r3, r2, r1=None, feature_shape=None):
        ...
        p = NF.interpolate(p, scale_factor=2,
                           mode='bilinear', align_corners=False)
        ...
        uncertainty = uncertainty.expand(-1, obj_n, -
                                         1, -1).reshape(bs * obj_n, 1, h, w)
...
class AFB_URR(nn.Module):
    ...
    def memorize(self, frame, mask):
        ...
        (frame, mask), pad = myutils.pad_divide_by(
    ...
    def segment(self, frame, fb_global):
        ...
        r3_size = r3.shape
        r2_size = r2.shape
        r3 = r3.unsqueeze(1).expand(-1, obj_n, -1, -1, -
                                    1).reshape(bs * obj_n, *r3_size[1:])
        r2 = r2.unsqueeze(1).expand(-1, obj_n, -1, -1, -
                                    1).reshape(bs * obj_n, *r2_size[1:])

        r1_size = r1.shape
        r1 = r1.unsqueeze(1).expand(-1, obj_n, -1, -1, -
                                    1).reshape(bs * obj_n, *r1_size[1:])
        ...
        if self.training:
            ...
            uncertainty = uncertainty.view(
                bs, -1).norm(p=2, dim=1) / math.sqrt(frame.shape[-2] * frame.shape[-1])  # [B,1,H,W]

...
def db_eval_iou(annotation, segmentation, void_pixels=None):
    ...
    inters = np.sum((segmentation & annotation) &
                    np.logical_not(void_pixels), axis=(-2, -1))
    union = np.sum((segmentation | annotation) &
                   np.logical_not(void_pixels), axis=(-2, -1))
...
def db_eval_boundary(annotation, segmentation, void_pixels=None, bound_th=0.008):
    ...
    if annotation.ndim == 3:
        ...
        f_res = f_measure(segmentation, annotation,
                          void_pixels, bound_th=bound_th)
    else:
        raise ValueError(
...
def f_measure(foreground_mask, gt_mask, void_pixels=None, bound_th=0.008):
    ...
    Returns:
        NF (float): boundaries NF-measure
    """

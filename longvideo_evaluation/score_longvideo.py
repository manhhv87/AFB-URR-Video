...
csv_name_per_sequence_path = os.path.join(
    method_results_path, csv_name_per_sequence)
if not args.update and os.path.exists(csv_name_global_path) and os.path.exists(csv_name_per_sequence_path):
    ...
    dataset_eval = Evaluation(root_folder=args.path,
                              task=args.task, filelist=args.filelist)
    metrics_res = dataset_eval.evaluate(method_results_path)
    J, F = metrics_res['J'], metrics_res['NF']

    # Generate dataframe for the general results
    g_measures = ['J&NF-Mean', 'J-Mean', 'J-Recall',
                  'J-Decay', 'NF-Mean', 'NF-Recall', 'NF-Decay']
...
# Print the results
sys.stdout.write(
    f"------------------------ Global results for {args.method} ------------------------\n")
